...
export default function ObservabilityPage() {
  const { logs, searchQuery } = useAppStore();

  const defaultLogs = [
    {
      stepId: 'step-101',
      agentName: 'Orchestrator Manager Agent',
      action: 'Decomposed user prompt into execution subtasks',
    ...
    {
      stepId: 'step-102',
      agentName: 'Playwright Browser Scraper Agent',
      action: 'Navigated to target URL https://news.ycombinator.com',
      reasoning: 'Extracted raw DOM tree, parsed top articles, taken screenshot snapshot.',
      status: 'SUCCESS',
      tokensUsed: 120,
      durationMs: 340,
      timestamp: new Date().toLocaleTimeString()
    ...
    {
      stepId: 'step-103',
      agentName: 'DevSecOps & Compliance Auditor',
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      <InstructionBanner
        ...
        steps={[
          "Ensure Stream Connection: Verify green 'Telemetry Active' badge at top right.",
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 rounded-2xl glass-panel border border-slate-800">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Live execution trace stream, step reasoning inspector, token counter, latency metrics, and cost tracking.
          </p>
        </div>
      ...
      <div className="p-6 rounded-2xl glass-panel border border-slate-800 space-y-4">
        ...
        {displayLogs.length === 0 ? (
          <div className="p-12 text-center text-slate-500 text-xs font-mono border border-dashed border-slate-800 rounded-2xl">
            No telemetry logs match search query. Run a workflow in the builder tab to generate live traces.
          </div>
        ) : (
