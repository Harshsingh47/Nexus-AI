...
export default function LoginPage() {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      const data = await res.json();
      setUser(data.user || mockProfile, data.token || 'google-jwt-token');
      router.push('/dashboard');
    } catch (e) {
      setUser({ id: 'usr-google', email: 'user.google@nexusmind.ai', fullName: 'Google User', role: 'MEMBER' }, 'google-token');
      router.push('/dashboard');
    } finally {
      setLoading(false);
    }
    ...
    <div className="min-h-screen bg-dark-950 flex items-center justify-center p-4 -ml-64 relative overflow-hidden">
      ...
      <div className="w-full max-w-md p-8 rounded-3xl glass-panel border border-slate-800 space-y-6 shadow-2xl bg-dark-950/90">
        <div className="text-center space-y-2">
          ...
          <h1 className="text-2xl font-extrabold text-white tracking-tight">Welcome to NexusMind</h1>
        ...
        <div className="grid grid-cols-2 p-1 rounded-xl bg-slate-900 border border-slate-800 text-xs font-semibold">
          <button
            ...
            className={`py-1.5 rounded-lg transition-all ${authMode === 'PASSWORD' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}
          >
            Email & Password
          </button>
          <button
            onClick={() => setAuthMode('MAGIC_LINK')}
            className={`py-1.5 rounded-lg transition-all ${authMode === 'MAGIC_LINK' ? 'bg-blue-600 text-white shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}
          >
            Magic Link
          </button>
        </div>
        ...
        <button
          ...
          disabled={loading}
          className="w-full py-2.5 px-4 rounded-xl bg-slate-900 hover:bg-slate-800 border border-slate-800 text-slate-200 text-xs font-semibold flex items-center justify-center gap-2.5 transition-all shadow-sm"
        >
        ...
        {authMode === 'PASSWORD' ? (
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="admin@nexusmind.ai"
          ...
          <form onSubmit={handleMagicLink} className="space-y-4">
            <div>
              <label className="text-xs text-slate-400 block mb-1">Email Address for Magic Link</label>
              <div className="relative">
                <Mail className="w-4 h-4 text-slate-500 absolute left-3 top-1/2 -translate-y-1/2" />
                <input
                  type="email"
            ...
            {magicLinkSent ? (
              <div className="p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-xs text-center font-mono">
                ✓ Magic authentication link sent to {email}!
              </div>
            ) : (
              <button
                type="submit"
                disabled={loading}
                className="w-full py-2.5 px-4 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-semibold text-xs flex items-center justify-center gap-2 transition-all shadow-md shadow-blue-600/20"
              >
                {loading ? <RefreshCw className="w-4 h-4 animate-spin" /> : <span>Send Passwordless Link</span>}
              </button>
            )}
          </form>
        )}
